...
import Jack from '@/components/molecules/Jack'
...
function PreviewCable({ from, preview }) {
  ...
  return (
    <path
      ...
      className={`${styles.cablePath} ${styles.preview}`}
    />
  )
}
...
function usePatchData(jackEntries, values, setValue) {
  ...
  const removeAll = useCallback((srcKey) => {
    ...
    if (!Array.isArray(srcConns))
      return
    srcConns.forEach(dstKey => setValue(dstKey, null))
...
export default function PatchBay({ params, values, setValue }) {
  ...
  return (
    <div
      ref={containerRef}
      className={`${styles.patchbay}${from ? ` ${styles.dragging}` : ''}`}
    ...
    >
      ...
      {jackEntries.map(([key, param]) => (
        <Jack
          key={key}
          data-jack={key}
          isConnected={jackIsConnected(key, param)}
          isActive={from && from.key === key}
